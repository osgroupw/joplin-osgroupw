...
export const runtime = (): CommandRuntime => {
	return {
		execute: async (context: CommandContext) => {
			...
			const currentNote = stateUtils.selectedNote(context.state);

			const link = currentNote?.markup_language === MarkupLanguage.Html
				? `<a href=":/${markdownUtils.escapeLinkUrl(result.item.id)}">${htmlentities(result.item.title)}</a>`
